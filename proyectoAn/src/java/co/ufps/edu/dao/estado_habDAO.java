...
public class estado_habDAO {
    ...
    public estado_habDAO() {
     ...
     public String insertar (estadohab estadoh){
        String msm="";
       ResultSet res= getCnn().consultaTabla("select max(id_tipo)+1 from estadohabitacion;");
       int id=0; 
       try {
           if(res.next()){
               if(res.getInt(1)==0){
    ...
    try {
        while (msm.next()){
         ...
         getCnn().cerrar();
        String sql="UPDATE estadohabitacion SET descripcion="+estadoh.getDescripcion()+" WHERE id_hab="+estadoh.getId()+";";
